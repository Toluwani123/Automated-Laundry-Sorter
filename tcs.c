...
static void tcs_pins_init(void){
    P2DIR |= (TCS_S0_PIN|TCS_S1_PIN|TCS_S2_PIN|TCS_S3_PIN);
    P2OUT |= (TCS_S0_PIN | TCS_S1_PIN);

    // OUT on P1.6 as GPIO input
    P1DIR  &= ~TCS_OUT_PIN;
    P1SEL0 &= ~TCS_OUT_PIN;
    P1SEL1 &= ~TCS_OUT_PIN;
    P1IES  &= ~TCS_OUT_PIN;  // rising edge
    P1IFG  &= ~TCS_OUT_PIN;  // clear flags
    P1IE   |=  TCS_OUT_PIN;  // enable interrupt
}
...
/* ---------------- Filter helpers ---------------- */
static inline void tcs_filter_red(void)   { P2OUT &= ~(TCS_S2_PIN|TCS_S3_PIN); }
static inline void tcs_filter_green(void) { P2OUT |=  (TCS_S2_PIN|TCS_S3_PIN); }
static inline void tcs_filter_blue(void)  { P2OUT &= ~TCS_S2_PIN; P2OUT |= TCS_S3_PIN; }
...
static unsigned long measure_avg_period(unsigned int samples, unsigned int timeout_us){
    ...
    while (got < samples){
        if (g_edges != last){
            last = g_edges;
            sum += (unsigned long)g_period;  // Cast to unsigned long
            got++;
            t0 = TA0R;
        }
...
void tcs3200_measure(unsigned long *red, unsigned long *green, unsigned long *blue) {
    // Reset edge counter for each color measurement
    g_edges = 0;
    g_last = TA0R;

    tcs_filter_red();
    dly(20000);
    unsigned long pr = measure_avg_period(40, 120000);  // Change to unsigned long
    *red = pr ? (1000000UL / pr) : 0;
    ...
    tcs_filter_green();
    dly(20000);
    unsigned long pg = measure_avg_period(40, 120000);  // Change to unsigned long
    *green = pg ? (1000000UL / pg) : 0;
    ...
    tcs_filter_blue();
    dly(20000);
    unsigned long pb = measure_avg_period(40, 120000);  // Change to unsigned long
    *blue = pb ? (1000000UL / pb) : 0;
}
